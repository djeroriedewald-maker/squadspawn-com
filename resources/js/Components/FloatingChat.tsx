...
export default function FloatingChat() {
    ...
    if (view === 'closed') {
        return (
            <button
                ...
                className="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-gaming-purple shadow-lg shadow-gaming-purple/30 transition-transform hover:scale-110 active:scale-95"
            >
    ...
    const renderNotifContent = (notif: Notification) => {
        ...
        switch (d.type) {
            ...
            case 'new_message':
                return <><strong>{d.sender_name}</strong><span className="text-gray-400">: {d.message_preview}</span></>;
            case 'lfg_accepted':
    ...
    return (
        <>
            ...
            {/* Panel */}
            <div className="fixed z-50 flex flex-col overflow-hidden border-white/10 bg-navy-800 shadow-2xl shadow-glow-purple inset-0 sm:inset-auto sm:bottom-5 sm:right-5 sm:h-[540px] sm:w-[380px] sm:rounded-2xl sm:border">
                    ...
                    <>
                        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => setView('closed')} className="rounded-lg p-1.5 text-gray-400 transition hover:bg-navy-700 hover:text-white">
                            ...
                            {([
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-2.5 text-center text-[11px] font-semibold transition ${
                                        tab === key ? 'border-b-2 border-gaming-purple text-gaming-purple' : 'text-gray-500 hover:text-gray-300'
                                    }`}
                                >
                                    {label}
                                    {count > 0 && (
                                        <span className={`ml-1 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full px-1 text-[9px] font-bold text-white ${key === 'online' ? 'bg-gaming-green' : 'bg-gaming-pink'}`}>
                                            {count > 9 ? '9+' : count}
                                        </span>
                                    )}
                                </button>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            {tab === 'chats' && (
                                friends.length === 0 ? (
                                    ...
                                    <button key={friend.id} onClick={() => openFriendChat(friend)} className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-navy-700 active:bg-navy-600">
                            ...
                            {tab === 'groups' && (
                                lfgGroups.length === 0 ? (
                                    ...
                                    <button key={group.id} onClick={() => openLfgChat(group)} className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-navy-700 active:bg-navy-600">
                                        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-navy-700">
                                            {group.game_cover ? (
                                                <img src={group.game_cover} alt="" className="h-full w-full object-cover" />
                                            ) : (
                                                <span className="text-[10px] font-bold text-gaming-purple">{(group.game_name || 'LFG')[0]}</span>
                                            )}
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center justify-between">
                                                <span className={`truncate text-sm ${group.unread_count > 0 ? 'font-bold text-white' : 'font-medium text-gray-300'}`}>
                                                    {group.title}
                                                </span>
                                ...
                                notifications.length === 0 ? (
                                    ...
                                    <>
                                        <div className="flex justify-end border-b border-white/5 px-4 py-2">
                                            <button onClick={handleMarkAllRead} className="text-[11px] text-gaming-purple hover:text-gaming-purple/80">Mark all read</button>
                                        </div>
                                        {notifications.map((notif) => (
                                            <button key={notif.id} onClick={() => handleNotificationClick(notif)} className="flex w-full items-center gap-3 border-b border-white/5 px-4 py-3 text-left transition hover:bg-navy-700 active:bg-navy-600">
                                                <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gaming-purple/20">
                                                    {getNotifAvatar(notif) ? (
                                                        ...
                                                        <span className="text-xs font-bold text-gaming-purple">{getNotifName(notif)[0].toUpperCase()}</span>
                                                    )}
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                    <p className="text-sm text-white">{renderNotifContent(notif)}</p>
                            ...
                            {tab === 'online' && (
                                onlineFriends.length === 0 ? (
                                    ...
                                    <button key={friend.id} onClick={() => openFriendChat(friend)} className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-navy-700 active:bg-navy-600">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <span className="text-sm font-medium text-white">{friend.partner.username || friend.partner.name}</span>
...
function AvatarWithStatus({ avatar, name, online, size = 10 }: { avatar?: string; name: string; online?: boolean; size?: number }) {
    return (
        <div className="relative shrink-0">
            <div className={`flex h-${size} w-${size} items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-gaming-purple/30 to-gaming-green/30 text-sm font-bold text-white`}>
                {avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : name[0]?.toUpperCase()}
            </div>
            {online && <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-navy-800 bg-gaming-green" />}
        </div>
    );
}
...
function EmptyState({ icon, text, action, onAction }: { icon: string; text: string; action?: string; onAction?: () => void }) {
    return (
        <div className="flex flex-col items-center justify-center px-4 py-12 text-center">
            ...
            {action && onAction && (
                <button onClick={onAction} className="mt-3 rounded-lg bg-gaming-purple/20 px-4 py-1.5 text-xs font-semibold text-gaming-purple transition hover:bg-gaming-purple/30">
                    {action}
                </button>
            )}
        </div>
...
function ChatHeader({ title, subtitle, subtitleColor, avatar, avatarName, online, isSquare, statusDot, onBack, onExpand, onClose }: {
    ...
    return (
        <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2.5">
            <button onClick={onBack} className="shrink-0 rounded-lg p-1 text-gray-400 transition hover:bg-navy-700 hover:text-white">
            ...
            <div className="relative shrink-0">
                <div className={`flex h-8 w-8 items-center justify-center overflow-hidden ${isSquare ? 'rounded-lg' : 'rounded-full'} bg-gradient-to-br from-gaming-purple/30 to-gaming-green/30 text-xs font-bold text-white`}>
                    {avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : avatarName[0]?.toUpperCase()}
                </div>
                {online && <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-navy-800 bg-gaming-green" />}
                {statusDot && <span className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-navy-800 ${statusDot}`} />}
            </div>
            <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-white">{title}</p>
                <p className={`text-[10px] ${subtitleColor}`}>{subtitle}</p>
            </div>
            <button onClick={onExpand} className="shrink-0 rounded-lg p-1.5 text-gray-400 transition hover:bg-navy-700 hover:text-white" title="Open full view">
            ...
            <button onClick={onClose} className="shrink-0 rounded-lg p-1.5 text-gray-400 transition hover:bg-navy-700 hover:text-white">
...
function MessageList({ messages, loading, userId, isGroup, endRef, formatTime, getMsgSender, getMsgSenderId }: {
    ...
    if (loading) {
        return <div className="flex flex-1 items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-gaming-purple border-t-transparent" /></div>;
    }
                    ...
                    return (
                        <div key={msg.id} className={isFirstInGroup && i > 0 ? 'mt-2' : 'mt-0.5'}>
                            ...
                            {isGroup && !isMe && isFirstInGroup && sender && (
                                <p className="mb-0.5 ml-1 text-[10px] font-semibold text-gaming-purple">
                            ...
                            <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                ...
                                {isGroup && !isMe && isFirstInGroup && (
                                    <div className="mr-1.5 flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full bg-navy-600 text-[9px] font-bold text-white">
                                ...
                                <div className={`max-w-[80%] rounded-2xl px-3 py-1.5 ${isMe ? 'bg-gaming-purple text-white' : 'bg-navy-700 text-gray-200'}`}>
                                    <p className="text-[13px] leading-relaxed">{msg.body}</p>
                                    <p className={`mt-0.5 text-right text-[9px] ${isMe ? 'text-white/40' : 'text-gray-600'}`}>{formatTime(msg.created_at)}</p>
                                </div>
                            </div>
                        </div>
                    );
...
function ChatInput({ textareaRef, onSubmit, onKeyDown, sending }: {
    ...
    return (
        <form onSubmit={onSubmit} className="border-t border-white/10 px-3 py-2">
            <div className="flex items-end gap-2">
                ...
                />
                <button type="submit" disabled={sending} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gaming-purple text-white transition hover:bg-gaming-purple/80 disabled:opacity-50">
