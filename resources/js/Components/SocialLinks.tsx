...
export default function SocialLinks({ socials }: { socials?: Record<string, string> }) {
    ...
    const getUrl = (key: string, value: string) => {
        if (key === 'discord') return undefined; // Discord is just a tag, not a link
    ...
    return (
        <div className="flex flex-wrap gap-2">
            {activeSocials.map(([key, value]) => {
                ...
                if (url) {
                    return (
                        <a key={key} href={url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-1.5 rounded-lg border border-white/10 px-2.5 py-1.5 transition ${config.color}`} title={config.label}>
                            {content}
                        </a>
                    );
                }
                ...
                return (
                    <span key={key} className={`flex items-center gap-1.5 rounded-lg border border-white/10 px-2.5 py-1.5 ${config.color.split(' ')[0]}`} title={config.label}>
                        {content}
                    </span>
                );
            })}
